...
import org.jeecg.modules.common.util.QRCodeUtils;
...
public class SysBaseApiImpl implements ISysBaseAPI {
    ...
    @Override
    // TODO: redis可能会core，不在考虑从redis中读取
    ...
    public void sendTemplateAnnouncement(TemplateMessageDTO message) {
        ...
        for (int i = 0; i < userIds.length; i++) {
            if (oConvertUtils.isNotEmpty(userIds[i])) {
                SysUser sysUser = userMapper.selectById(userIds[i]);
                if (sysUser == null) {
                    continue;
                }
...
//			}
    ...
    public Map<String, String> generateQRCode() {
        ...
        map.put("imageId", content);
        BufferedImage image = null;
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            image = QRCodeUtils.createImage(content, null, true);
            ImageIO.write(image, "jpg", baos);
            String imageStr = Base64.getEncoder().encodeToString(baos.toByteArray());
     ...
     */
    @Override
    public String imageToBinary(String url) {
        ...
        try {
            ...
            String imgType=HttpURLConnection.guessContentTypeFromStream(bis);
            if ("image/gif".equals(HttpURLConnection.guessContentTypeFromStream(bis))){
                return null;
            }
