...
public class SysDepartPermissionController extends JeecgController<SysDepartPermission, ISysDepartPermissionService> {
	...
	public Result<?> edit(@RequestBody SysDepartPermission sysDepartPermission) {
	    LambdaQueryWrapper<SysDepartPermission> qw = new LambdaQueryWrapper<SysDepartPermission>();
	    qw.eq(SysDepartPermission::getDepartId,sysDepartPermission.getDepartId());
        SysDepartPermission sysDepartPermissions = sysDepartPermissionService.getOne(qw);
        if(sysDepartPermissions == null){
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
	    LambdaQueryWrapper<SysDepart> qw = new LambdaQueryWrapper<SysDepart>();
	    qw.eq(SysDepart::getId,id);
        SysDepart depart = sysDepartService.getOne(qw);
        if(depart == null){
            return Result.error("删除失败，id不存在");
        }
        LambdaQueryWrapper<SysDepartPermission> qw1 = new LambdaQueryWrapper<SysDepartPermission>();
        qw1.eq(SysDepartPermission::getDepartId,id);
        sysDepartPermissionService.remove(qw1);
