...
public class SysDepartController {
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-我的部门", notes = "查询数据-我的部门")
    ...
    public Result<List<SysDepartTreeModel>> queryMyDeptTreeList() {
        ...
        try {
            if (oConvertUtils.isNotEmpty(user.getUserIdentity()) && user.getUserIdentity().equals(CommonConstant.USER_IDENTITY_2)) {
                sysUserDepartService.queryDepartIdsOfUser(user.getId());
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-所有部门", notes = "查询数据-所有部门")
...
//			}
     ...
     */
    //@RequiresRoles({"admin"})
    @ApiOperation(value = "添加数据-添加部门", notes = "添加数据-添加部门")
     ...
     */
    //@RequiresRoles({"admin"})
    @ApiOperation(value = "修该数据-修改部门", notes = "修该数据-修改部门")
     ...
     */
    //@RequiresRoles({"admin"})
    @ApiOperation(value = "删除数据-通过id删除部门", notes = "删除数据-通过id删除部门")
     ...
     */
    //@RequiresRoles({"admin"})
    @ApiOperation(value = "批量删除数据-通过id删除部门", notes = "批量删除数据-通过id删除部门")
    ...
    public Result<SysDepart> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        ...
        for (int i = 0; i < Arrays.asList(ids.split(",")).size(); i++) {
            if (sysDepartService.getById(Arrays.asList(ids.split(",")).get(i)) != null) {
                sign += 1;
            }
        }
        if (sign > 0) {
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-添加或编辑页面对该方法发起请求", notes = "查询数据-添加或编辑页面对该方法发起请求")
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-部门搜索", notes = "查询数据-部门搜索")
    ...
    public Result<List<SysDepartTreeModel>> searchBy(@RequestParam(name = "keyWord", required = true) String keyWord, @RequestParam(name = "myDeptSearch", required = false) String myDeptSearch) {
        ...
        List<SysDepartTreeModel> treeList = this.sysDepartService.searchBy(keyWord, myDeptSearch, departIds);
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-查询所有部门信息", notes = "查询数据-查询所有部门信息")
    @GetMapping("listAll")
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-keyWord查询所有部门", notes = "查询数据-keyWord查询所有部门")
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-根据部门编码获取部门信息", notes = "查询数据-根据部门编码获取部门信息")
     ...
     * @return
     */
    @ApiOperation(value = "查询数据-根据部门id获取用户信息", notes = "查询数据-根据部门id获取用户信息")
