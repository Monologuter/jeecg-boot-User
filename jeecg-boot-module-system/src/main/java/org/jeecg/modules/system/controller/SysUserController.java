...
import org.jeecg.modules.system.mapper.SysMemberShipMapper;
import org.jeecg.modules.system.mapper.SysUserMapper;
import org.jeecg.modules.system.model.DepartIdModel;
import org.jeecg.modules.system.model.SysUserSysDepartModel;
import org.jeecg.modules.system.properties.UserProperties;
...
import org.jeecg.modules.workflow.mapper.WorkFlowActTenantMember;
...
public class SysUserController {
    ...
    private Object ActMemberShipVO;
    ...
    @Autowired
    private WorkFlowActTenantMember workFlowActTenantMember;
    ...
    public Result<IPage<SysUser>> queryPageList(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        queryWrapper.inSql("id", "SELECT su.id FROM sys_user su WHERE REPLACE ( su.rel_tenant_ids, ',', ' ' ) REGEXP '\\\\b" + tenantId + "\\\\b'");
    ...
    public Result<SysUser> add(@RequestBody SysUserVo sysUserVo) {
        ...
        String selectedRoles = sysUserVo.getSelectedroles();
        String selectedDeparts = sysUserVo.getSelecteddeparts();
        try {
            if (CheckAttrUtil.isNull(sysUserVo, "username", "realname", "password", "workNo")) {
                return result.error500("操作失败");
            }
            sysUserVo.setCreateTime(new Date());//设置创建时间
            String salt = oConvertUtils.randomGen(8);
            sysUserVo.setSalt(salt);
            String passwordEncode = PasswordUtil.encrypt(sysUserVo.getUsername(), sysUserVo.getPassword(), salt);
            sysUserVo.setPassword(passwordEncode);
            sysUserVo.setStatus(1);
            sysUserVo.setDelFlag(CommonConstant.DEL_FLAG_0);
            if (oConvertUtils.isEmpty(sysUserVo.getAvatar())) {
                sysUserVo.setAvatar(UserProperties.DEFAULT_AVATARD);
            }
            sysUserVo.setRelTenantIds(TenantContext.getTenant());
            sysUserService.addUserWithRole(sysUserVo, selectedRoles);
            sysUserService.addUserWithDepart(sysUserVo, selectedDeparts);
            user1.setUserId(sysUserVo.getId());
            user1.setEmail(sysUserVo.getEmail());
            user1.setFirstName(sysUserVo.getRealname());
            user1.setLastName(sysUserVo.getRealname());
            wfUserService.addUser(user1);
//            将用户信息同步到camunda用户表 以上是进行字段对应赋值

            //扩展用户信息表插入头像字段
            String s1 = sysUserVo.getId();
            String s2 = "avatar";
            String s3 = sysUserVo.getAvatar();
            ProcessEngineUtil.PROCESS_ENGINE_INSTANCE.getIdentityService().setUserInfo(s1, s2, s3);
            result.success("添加成功！");
        } catch (Exception e) {
    ...
    public Result<SysUser> edit(@RequestBody SysUserVo sysUserVo) {
        try {
            ...
            userDTO.setUserId(sysUserVo.getId());
            userDTO.setEmail(sysUserVo.getEmail());
            userDTO.setFirstName(sysUserVo.getRealname());
            userDTO.setLastName(sysUserVo.getRealname());
            wfActIdUserMapper.updateById(userDTO);
            //修改扩展属性表用户头像信息
            String s1 = sysUserVo.getId();
            String s2 = "avatar";
            String s3 = sysUserVo.getAvatar();
            ProcessEngineUtil.PROCESS_ENGINE_INSTANCE.getIdentityService().setUserInfo(s1, s2, s3);
            sysUserService.editUser(sysUserVo);
            return Result.OK("编辑成功", sysUserVo);
        } catch (Exception e) {
    ...
    public Result<?> delete(@RequestParam(name = "id", required = true) String id) {
        // 判断所选用户是否存在
        if (oConvertUtils.isEmpty(sysUserService.getById(id))) {
    ...
    public Result<String> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        ...
        for (String userId : userIds) {
            if (Objects.isNull(sysUserService.getById(userId))) {
                notExist.add(userId);
            }
        }
        userIds.removeAll(notExist);
        baseCommonService.addLog("批量删除用户， userIds： " + userIds, CommonConstant.LOG_TYPE_2, 3);
        if (notExist.size() == 0) {
            this.sysUserService.deleteBatchUsers(userIds);
            return Result.OK("所有用户被删除!");
        } else if (userIds.size() > 0) {
            this.sysUserService.deleteBatchUsers(userIds);
            return Result.OK("部分用户被删除!不存在的id有" + notExist);
        } else {
            return Result.Error("所有用户id错误");
        }
    }
    ...
    public Result<SysUser> frozenBatch(@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            if (oConvertUtils.isEmpty(arr[0])) {
                result.error500("未选中用户");
                return result;
            }
            for (String id : arr) {
     ...
     * @return
     */
    @ApiOperation("成在添加用户情况下没有主键的问题,返回给前端,根据该id绑定部门数据")
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            if (!(fileType.equals(".xls") || fileType.equals(".xlsx"))) {
                return Result.Error("不支持该格式文件导入");
            }
            ...
            try {
                ...
                for (int i = 0; i < listSysUsers.size(); i++) {
                    ...
                    if (CheckAttrUtil.isAllNull(sysUserExcel)) {
                        continue;
                    }
                    ...
                    //只能将用户导入到当前租户下
                    String tenantId = TenantContext.getTenant();
                    sysUserExcel.setRelTenantIds(tenantId);
                    try {
    ...
    public Result<SysUserRole> deleteUserRole(@RequestParam(name = "roleId") String roleId,
        ...
        try {
            ...
            // 判断关系是否存在
            if (oConvertUtils.isEmpty(sysUserRoleService.getOne(queryWrapper))) {
    ...
    public Result<SysUserRole> deleteUserRoleBatch(
        ...
        try {
            ...
            // 判断关系是否存在
            if (oConvertUtils.isEmpty(sysUserRoleService.getMap(queryWrapper))) {
                result.error500("删除失败！");
            } else {
                sysUserRoleService.remove(queryWrapper);
                result.success("删除成功!");
     ...
     */
    @ApiOperation("部门用户列表")
    @RequestMapping(value = "/selectDepartUserList", method = RequestMethod.GET)
    public Result<IPage<SysUser>> selectDepartUserList(@RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                                       @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        Result<IPage<SysUser>> result = new Result<IPage<SysUser>>();
        String depId = req.getParameter("depId");
        //根据部门ID查询,当前和下级所有的部门IDS
        ...
        if (subDepids != null && subDepids.size() > 0) {
            ...
            QueryWrapper<SysUser> sysUserQueryWrapper = QueryGenerator.initQueryWrapper(sysUser, req.getParameterMap());
            Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);
            String join = StringUtils.join(subDepids.toArray(), "' , '");
            sysUserQueryWrapper.apply("id in (select user_id from sys_user_depart where dep_id in ( '" + join + "' ))");
            IPage<SysUser> sysUserPage = sysUserService.page(page, sysUserQueryWrapper);
     ...
     */
    @ApiOperation("根据 orgCode 查询用户(针对通讯录模块)")
...
//             json.put("avatar", item.getSysUser().getAvatar());
        ...
        IPage<JSONObject> result = new Page<>(pageNo, pageSize, pageList.getTotal() - count);
        result.setRecords(resultJson.toJavaList(JSONObject.class));
        return Result.ok(result);
    }

    @ApiOperation("根据 orgCode 查询用户(针对通讯录模块)")
    @GetMapping("/selectQueryByOrgCodeForAddressList")
    public Result<?> selectQueryByOrgCodeForAddressList(
            @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
            @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
            SysUser userParams,
        ...
        String orgCode = userParams.getOrgCode();
        userParams.setOrgCode(null);
        QueryWrapper<SysUser> queryWrapper = QueryGenerator.initQueryWrapper(userParams, req.getParameterMap());
        List<SysUserDepart> sysUserDeparts = null;
        ...
        if (Objects.nonNull(orgCode)) {
            ...
            departQueryWrapper.likeRight(SysDepart::getOrgCode, orgCode);
            List<SysDepart> sysDeparts = sysDepartService.list(departQueryWrapper);
            deptIds = sysDeparts.stream().map(SysDepart::getId).collect(Collectors.toList());
            LambdaQueryWrapper<SysUserDepart> uDQueryWrapper = new LambdaQueryWrapper<>();
            uDQueryWrapper.in(SysUserDepart::getDepId, deptIds);
            sysUserDeparts = sysUserDepartService.list(uDQueryWrapper);
            sysUserIds = sysUserDeparts.stream().map(SysUserDepart::getUserId).collect(Collectors.toList());
            queryWrapper.in("id", sysUserIds);
        }
        Page<SysUser> userPage = sysUserService.page(page, queryWrapper);
        List<String> userIds = userPage.getRecords().stream().map(SysUser::getId).collect(Collectors.toList());
        if (userIds != null && userIds.size() > 0) {
            Map<String, String> useDepNames = sysUserService.getDepNamesByUserIds(userIds);
    ...
    public Result<SysUserDepart> deleteUserInDepart(@RequestParam(name = "depId") String depId,
        ...
        try {
            ...
            if (b) {
                List<SysDepartRole> sysDepartRoleList = departRoleService.list(new LambdaQueryWrapper<SysDepartRole>()
                        .eq(SysDepartRole::getDepartId, depId));
                ...
                List<SysUserDepart> sysUserDeparts = sysUserDepartService.list(new LambdaQueryWrapper<SysUserDepart>()
                        .eq(SysUserDepart::getUserId, userId));
                List<String> depIds = sysUserDeparts.stream().map(SysUserDepart::getDepId).collect(Collectors.toList());
                if (depIds != null && depIds.size() > 0) {
                    StringBuilder str = new StringBuilder();
                    depIds.forEach(item -> str.append(item + ","));
                    SysUser sysUser = sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getId, userId));
                    sysUser.setDepartIds(str.deleteCharAt(str.length() - 1).toString());
     ...
     * @return
     */
    @ApiOperation("查询当前用户的所有部门/当前部门编码")
     ...
     * @return
     */
    @ApiOperation("用户通过手机号修改密码")
    @PostMapping("/updatePasswordByPhone")
    public Result<SysUser> updatePasswordByPhone(@RequestParam(name = "password") String password,
                                                 @RequestParam(name = "smscode") String smscode,
                                                 @RequestParam(name = "phone") String phone) {
        Result<SysUser> result = new Result<SysUser>();
        SysUser sysUser = sysUserService.updatePasswordByPhone(phone, smscode, password);
        if (sysUser == null) {
     ...
     * @return
     */
    @ApiOperation("根据TOKEN获取用户的部分信息s")
     ...
     */
    @ApiOperation("获取被逻辑删除的用户列表，无分页")
    @GetMapping("/recycleBin")
    public Result getRecycleBin() {
        List<SysUser> AllDeleList = sysUserService.queryLogicDeleted();
        List<SysUser> logicDeletedUserList = new ArrayList<SysUser>();
        //获取租户id
        String tenantId = TenantContext.getTenant();
        for (SysUser sysUser : AllDeleList) {
            String relTenantIds = sysUser.getRelTenantIds();
            List<String> ids = new ArrayList<String>(Arrays.asList(relTenantIds.split(",")));
            if (ids.contains(tenantId)) {
                logicDeletedUserList.add(sysUser);
            }
        }
    ...
    public Result deleteRecycleBin(@RequestParam("userIds") String userIds) {
        if (StringUtils.isNotBlank(userIds)) {
            ...
            // 彻底删除用户的同时同步删除membership表的信息
            // 对于批量删除需要将传递过来的多用户信息用逗号对ids进行分割
            String[] arr = userIds.split(",");
            for (int i = 0; i < arr.length; i++) {
                String id = arr[i];
                //自定义mapper 根据userid删除groupid
                workFlowActTenantMember.deleteTenantById(id);
                sysMemberShipMapper.deleteMembershipById(id);
                sysUserMapper.deleteUser(id);
                //删除用户扩展信息表关联
                ProcessEngineUtil.PROCESS_ENGINE_INSTANCE.getIdentityService().deleteUserInfo(id, "avatar");
            }
     ...
     * @return Result
     */
    @ApiOperation("根据userid获取用户信息和部门员工信息")
     ...
     * @return
     */
    @ApiOperation("移动端查询用户信息(通过用户名模糊查询)")
    ...
    public Result<List<SysUser>> queryByKeyword(String keyword) {
        Result<List<SysUser>> result = new Result<List<SysUser>>();
