    ...
    public List<SysDepartTreeModel> searchBy(String keyWord, String myDeptSearch, String departIds) {
        ...
        if (departList.size() > 0) {
            for (SysDepart depart : departList) {
                if (contains(sysDepartList, depart)) {
                    continue;
                }
                sysDepartList.add(depart);
                sysDepartList = getParentDepart(sysDepartList, depart);
            }
            newList = FindsDepartsChildrenUtil.wrapTreeDataToTreeList(sysDepartList);
            return newList;
        }
        return null;
    }

    private List<SysDepart> getParentDepart(List<SysDepart> sysDepartList, SysDepart depart) {
        SysDepart parentDepart = getById(depart.getParentId());
        if (!StringUtil.isNullOrEmpty(depart.getParentId()) && !contains(sysDepartList, parentDepart)) {
            sysDepartList.add(parentDepart);
            sysDepartList = getParentDepart(sysDepartList, parentDepart);
        }
        return sysDepartList;
    }

    private boolean contains(List<SysDepart> sysDepartList, SysDepart depart) {
        for (int i = 0; i < sysDepartList.size(); i++) {
            System.out.println(sysDepartList.get(i).getId());
            System.out.println(depart.getId());
            if (sysDepartList.get(i).getId().equals(depart.getId())) {
                return true;
            }
        }
        return false;
